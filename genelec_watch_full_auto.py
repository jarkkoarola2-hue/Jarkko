#!/usr/bin/env python3
"""
Genelec full-auto watcher

Monitors selected auction / used marketplaces for new Genelec listings and
sends notifications to WhatsApp (Twilio), Discord webhook, email, or stdout.

Designed to be safe by default:
- Tori disabled by default because recurring automated collection is restricted.
- Facebook Marketplace not included in automated scraping.

Usage:
  python genelec_watch_full_auto.py --once
  python genelec_watch_full_auto.py --interval 1800
  python genelec_watch_full_auto.py --config config_genelec_watch.json
"""
...
def select_attr(el, attr: str) -> Optional[str]:
    if not el:
        return None
    val = el.get(attr)
...
def scrape_kiertonet(fetcher: Fetcher, keywords: list[str]) -> list[Listing]:
    # Search URLs vary over time, so fallback to site search pattern and parse best-effort.
    candidate_urls = [
...
def scrape_tori(fetcher: Fetcher, keywords: list[str]) -> list[Listing]:
    # Disabled by default via config. Best-effort only.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Full-auto Genelec watcher")
